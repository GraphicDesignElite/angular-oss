...
export class SearchViewComponent implements OnInit {
  ...
  isSearchView: boolean = false; // Modifies Visual Output Only After Typing
  ...
  constructor(private _solrService: SolrService, private _displayService: DisplayService, public _route: ActivatedRoute, private _router: Router) {
  ...
  isSearching() { // when a user types, the event is called and this fires to change the application appearance
    if (this.searchQuery != '' && !this.isSearchView) {
      this._displayService.showSearchView();
    }
  }
